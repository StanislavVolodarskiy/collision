...
пролетают сквозь друг друга. `completeCollision` проверяет все пары, работает
не быстро. `makeOptimizedCollision` ускоряет проверку пересечения с помощью
quadtree, про неё ниже.
...
Параметры:
* `width`, `height` &ndash; ширина и высота прямоугольника. Можно было бы обойтись без
них, но тогда надо будет восстанавливать прямоугольник каждый кадр. А так
меньше кода и работает чуть быстрее.
