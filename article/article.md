...
составим список шаров, которые с ним пересекаются. Большинство шаров
пересекутся с каким-то одним квадрантом, некоторые пересекутся с двумя,
единицы пересекутся с тремя или четырьмя. Процесс разбиения будем повторять
рекурсивно, пока списки не станут короче некоторого порога или пока глубина
рекурсии не превысит установленного предела. В каждом списке из листа дерева
надо будет проделать попарную проверку пересечения. Так как мы рассчитываем
что списки не длинные, то и проверка будет быстрой. Глубина дерева ограничена
чтобы избежать ситуаций когда много шаров попадают во много квадрантов и
списки почти не укорачиваются при разбиении.
...
попасть в два или больше квадрантов, тогда она попадёт в эту сумму несколько
...
[benchmark.js](../benchmark.js) расчитывает столкновения без отрисовки.
*&Delta;t* соответствует *60* FPS.
...
```
1000 0.020705977976322173 732995 1316061 18992 14766
```
...
Среднее время обработки пересечений около *20* миллисекунд. Примерно *50* FPS.

## Код

[lib/physics.js](../lib/physics.js) отвечает за симуляцию движения шаров.

Функция `update()` сдвигает шары согласно их скоростям и выполняет проверку
столкновений и меняет скорости шаров если они столкнулись друг с другом или со
стенками прямоугольника. Параметры:
* `width`, `height` &ndash; ширина и высота прямоугольника внутри которого
двигаются шары;
* `dt` &ndash; шаг по времени;
* `collision` &ndash; процедура проверки столкновения шаров. Бывает трёх видов:
* * `noCollision` совсем не проверят столкновение между шарами, в итоге шары
пролетают сквозь друг друга.
* * `completeCollision` проверяет все пары, работает
не быстро.
* * `makeOptimizedCollision` ускоряет проверку пересечения с помощью quadtree,
про неё ниже.
* `balls` &ndash; массив состояний шаров. `update` читает данные из него и
записывает обновлённые состояния в него же. Смотрите `makeBall`.
* `stats` &ndash; здесь накапливается статистика по столкновениям. Ею
пользуются чтобы подбирать параметры дерева для наилучшей производительности.
Смотрите `makeStats`.

`makeOptimizedCollision` строит оптимизированную процедуру пересечения шаров.
Параметры:
* `width`, `height` &ndash; ширина и высота прямоугольника. Можно было бы
обойтись без них, но тогда надо будет восстанавливать прямоугольник каждый
кадр. А так меньше кода и работает чуть быстрее.
* `maxDepth` ограничивает глубину quadtree, влияет на скорость;
* `maxLeafSize` ограничивает число шаров листьях quadtree, влияет на скорость.

`makeBall` строит объект шар. Параметры соответствуют полям объекта:
* `mass` &ndash; масса шара. На пересечение не влияет, влияет на расчёт
отскока шаров при столкновении;
* `radius` &ndash; радиус шара;
* `x`, `y` &ndash; позиция центра шара, её обновляет `update`;
* `vx`, `vy` &ndash; скорость шара, её тоже обновляет `update`.

`makeStats` строит объект для хранения статистики расчтёта столкновений. В
новой статистике все значения нулевые. `update` на каждом вызове увеличивает
счётчики в статистике.  Параметры соответствуют полям объекта:
* `splits` &ndash; суммарное количество шаров, которые через операцию
разбиения квадрата на квадранты. Это число пропорционально времени построения
quadtree. Если это число разделить на число шаров получим среднюю глубину
quadtree.
* `tests` &ndash; количество попарных тестов расстояния между шарами. Для
снижения этой метрики всё дерево и задумывалось. Если это число разделить на
число шаров, получится среднее количество шаров в листе quadtree.
* `collisions` &ndash; число пар которые действительно пересеклись. Это число
немного завышает действительное число пересечений: при делении на квадранты
одна пара может попасть в два или больше квадрантов, тогда она попадёт в эту
сумму несколько раз. Но скорее всего эта сумма не намного завышает число
пересечений.
* `reactions` &ndash; число раз когда были изменены скорости столкнувшихся
шаров. Столкновение отрабатывается только если шары сближаются. Если они
отдаляются, менять скорости нет смысла. Благодаря этому пару шаров можно
отправлять на обработку столкновения несколько раз, а скорости будут изменены
только один раз.

[html/index.html](../html/index.html) &ndash; показывает симуляцию в браузере.
Вам понадобится простенький http-сервер. Симуляция та же что и в
[демонстрации](https://stanislavvolodarskiy.github.io/collision/).

[benchmark.js](../benchmark.js) гоняет симуляцию без браузера и печатает
статистику для каждого кадра. Это node.js скрипт с позиционными параметрами:
1. `width` &ndash; ширина прямоугольника;
2. `height` &ndash; высота прямоугольника;
3. `n` &ndash; число шаров;
4. `maxRadius` &ndash; максимальный радиус шара;
5. `minRadius` &ndash; минимальный радиус шара;
6. `v` &ndash; каждый шар при рождении получает скорость в случайном
направлении, это её модуль;
7. `mode` &ndash; режим поиска пересечений. `0` &ndash; без пересечений,
`1` &ndash; проверка всех пар без оптимизаций, `2` &ndash; оптимизация
quadtree;
8. `maxDepth` &ndash; ограничивает глубину дерева;
9. `maxLeafSize` &ndash; ограничивает число шаров в листе дерева.
